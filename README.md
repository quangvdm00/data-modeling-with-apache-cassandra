...
1. **`song_info_by_session_and_item`** includes artist, song title and song length information for a given `sessionId` and `itemInSessionId`.

2. **`song_info_by_id_and_sessionId`** includes artist, song and user for a given `userId` and `sessionId`.
